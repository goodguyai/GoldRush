 ...
 * 
 * 1. First, import and run diagnosis:
 *    const { diagnoseLeague, repairLeague, repairAllLeagues } = await import('/src/utils/emergencyRepair');
 *    // OR simply access via window since it is attached
 *    await window.diagnoseLeague('YOUR_LEAGUE_ID');
 * 
 * 2. Then repair:
 *    await window.repairLeague('YOUR_LEAGUE_ID');
 * 
 * 3. Or repair ALL leagues:
 *    await window.repairAllLeagues();
 */
...
/**
 * Diagnose a single league for issues
 */
export const diagnoseLeague = async (leagueId: string): Promise<DiagnosisResult> => {
  ...
  // Check if commissioner has team doc
...
export const repairLeague = async (leagueId: string): Promise<{ fixed: string[], failed: string[] }> => {
  ...
  for (const wave of diagnosis.waves) {
    ...
    try {
      if (!wave.exists) {
        ...
        // Add missing users to existing wave
  ...
  // Create commissioner team doc if missing
...
/**
 * Repair ALL leagues in the database
 */
