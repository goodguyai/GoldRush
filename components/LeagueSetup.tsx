...
const LeagueSetup: React.FC<LeagueSetupProps> = ({ onComplete, onCancel, initialMode, userId }) => {
  ...
  const renderHeader = () => (
    <div className="text-center space-y-5 mb-8 pt-8 flex-shrink-0 relative">
      <button 
        onClick={onCancel}
        className="absolute top-8 left-6 text-gray-400 hover:text-red-500 transition-colors flex items-center gap-1.5"
      >
        <ArrowRight className="rotate-180" size={16} />
        <span className="text-xs font-bold uppercase tracking-wider">Lobby</span>
      </button>

      <div className="w-20 h-20 bg-white rounded-[24px] mx-auto flex items-center justify-center shadow-lg mb-6 text-electric-600 border border-gray-100">
        <OlympicRings size={50} />
      </div>
      <div>
        <h1 className="text-3xl font-black italic uppercase tracking-tighter text-gray-900">Gold Hunt '26</h1>
        <p className="text-gray-500 font-bold uppercase tracking-[0.2em] text-[10px] mt-2">Olympic Fantasy League</p>
      </div>
    </div>
  );
